...
public class RepsServiceImpl implements RepsService {
	...
	public Reps create(Reps reps) {
        ...
        if(taskses != null) {
            for(Tasks _tasks : taskses) {
                _tasks.setReps(repsCreated);
                LOGGER.debug("Creating a new child Tasks with information: {}", _tasks);
                tasksService.create(_tasks);
            }
        }

        if(quoteses != null) {
            for(Quotes _quotes : quoteses) {
                _quotes.setReps(repsCreated);
